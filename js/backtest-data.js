...
export async function fetchHistoricalUniverse({
  ...
  const load = async closed => {
    const params = new URLSearchParams({ ...base, closed: String(closed) });
    ...
    return (Array.isArray(payload) ? payload : []).map(marketFromGamma).filter(Boolean);
  };
...
export function buildDatasetFromSeries(series, marketDefs, { fidelityMinutes = 60 } = {}) {
  ...
  for (const timestamp of timeline) {
    ...
    for (const [marketId, market] of marketById) {
      ...
      for (const outcomeDef of market.outcomes) {
        ...
        if (lastPrices.has(key)) outcomes.push({ name: outcomeDef.name, price: lastPrices.get(key) });
      }
...
export async function fetchHistoricalDataset(markets, {
  ...
  if (universeMode === 'current') {
    ...
    universeMeta = { mode: 'historical', candidates: historical.candidates, selection: historical.selection };
  }
  ...
  const dataset = buildDatasetFromSeries(usableSeries, marketDefs, { fidelityMinutes });
...
export const _internal = { cleanHistory, eligibleMarkets, mapWithConcurrency, parseArray, hashSelectionKey, marketFromGamma };
